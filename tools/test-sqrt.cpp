...
double get_pagerank(EdgeTriplet<EdgeType> a){
    return min(pagerank[a.from],pagerank[a.to])/sqrt(a.value);
}
...
//      edges[i].value=1.0*i/10+1;
      smin=min(smin,(double)edges[i].value);
      smax=max(smax,(double)edges[i].value);
  }
//  for(uint64_t i=0;i<tot_edge_cnt;i++) edges[i].value=(edges[i].value-smin)/(smax-smin)+1;
  ...
  for(int i=0;i<10;i++){
    ...
    for(uint64_t j=0;j<edges.size();j++){
        pagerank[edges[j].to]+=tmp_p[edges[j].from]/deg[edges[j].from];
        //pagerank[edges[j].to]+=pagerank[edges[j].from]/edges[j].value/deg[edges[j].from];
