...
class DisjointSet {
public:
  IndexType FindRoot(IndexType x);
  bool Union(IndexType x, IndexType y);

private:
  std::unordered_map<IndexType, IndexType> root;
};

IndexType DisjointSet::FindRoot(IndexType x) {
  std::vector<IndexType> v{x};
  IndexType ans;
  for (;;) {
    auto ptr = root.find(x);
    if (ptr == root.end()) {
      ans = x;
      break;
    }
...
bool DisjointSet::Union(IndexType x, IndexType y) {
  // return true;
  IndexType rx = FindRoot(x);
  IndexType ry = FindRoot(y);
...
struct AdjCellP {
  ...
  void FromEdgeTriplets(vector<EdgeTriplet<EdgeType> >& vec, size_t start, size_t cnt) {
    ...
    if (cnt == 0) { start_vertex = end_vertex = start_dst_vertex = end_dst_vertex = edge_cnt = 0; return;}
...
double get_pagerank(EdgeTriplet<EdgeType> a){
    return min(pagerank[a.from],pagerank[a.to]);
}
...
vector<EdgeTriplet<EdgeType>>edge_grid[10][10];
int main(int argc, char ** argv) {
  ...
  cerr << "abstract_size " << abstract_size << "\n";
  ...
  //  Edge
  // *******************
  string info_file_path(output_prefix + "-W" + std::to_string(grid_width) + ".reachbility_info");
  ...
  size_t edge_size = sizeof(IndexType) * 2 + 0*sizeof(EdgeType);
  ...
  // set<pair<IndexType, IndexType> > ee;
  // for (uint64_t i = 0; i < tot_edge_cnt; ++i) {
  //   EdgeTriplet<EdgeType> e = edges[i];
    
  //   IndexType from = e.from;
  //   IndexType to = e.to;
  //   if (from > to) swap(from, to);
  //   ee.insert(make_pair(from, to));
  // }
  
  
  tot_edge_cnt = edges.size() * 2;
  ...
  vector<EdgeTriplet<EdgeType> > abstract;
  {/*
    vector<EdgeTriplet<EdgeType> > remained_edges;
    DisjointSet s;
    for (auto e: edges) {
      // if (abstract.size() < abstract_size && s.Union(e.from, e.to)) {
      //   abstract.push_back(e);
      //   IndexType xx = e.to;
      //   e.to = e.from;
      //   e.from = xx;
      //   abstract.push_back(e);
  ...
  for (; abstract.size() < abstract_size; ++erase_cnt) {
    EdgeTriplet<EdgeType> e = edges[edges.size() - 1 - erase_cnt];
    abstract.push_back(e);
    IndexType xx = e.to;
    e.to = e.from;
    e.from = xx;
  ...
  edges.resize(edges.size() - erase_cnt);
  cerr << abstract.size() << "===\n";
  ...
  //edge_grid = vector<vector<vector<EdgeTriplet<EdgeType> > > >(barrier.size() - 1, vector<vector<EdgeTriplet<EdgeType> > >(barrier.size() - 1));
  for (uint64_t i=edges.size()-1;i>=0;i--) {
    auto e= edges[i];
  ...
  vector<tuple<int, int, size_t, size_t> > cells;
  for (int i = 0; i < barrier.size() - 1; ++i) {
    for (int j = 0; j < barrier.size() - 1; ++j) {
      if (edge_grid[i][j].size() == 0) continue;
      random_shuffle(edge_grid[i][j].begin(), edge_grid[i][j].end());
      ...
      for (size_t start = 0; start < edge_grid[i][j].size(); start += step) {
        ...
        cells.emplace_back(i, j, start, end);
      }
    }
  }
  ...
  string edge_file_path(output_prefix + "-W" + std::to_string(grid_width) + ".reachbility_edge");
  ...
  for (auto& c: cells) {
    int i, j; size_t start, end;
    std::tie(i, j, start, end) = c;
