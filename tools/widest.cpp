...
struct AdjCellP {
  ...
  void FromEdgeTriplets(vector<EdgeTriplet<EdgeType> >& vec, size_t start, size_t cnt) {
    ...
    for (size_t i = start; i < start + cnt; ++i) {
      ...
      min_edge = max(min_edge, vec[i].value); //MIN->MAX
    }
    index.push_back(cnt);
...
int main(int argc, char ** argv) {
  ...
  for(uint64_t i=0;i<edges.size();i++) edges[i].value=1.0*i/10;
  ...
  //sort(edges.begin(), edges.end(), EdgeTripletGreaterByValue<EdgeType>());
  ...
  for (int i = 0; i < barrier.size() - 1; ++i) {
    for (int j = 0; j < barrier.size() - 1; ++j) {
      ...
      sort(edge_grid[i][j].begin(), edge_grid[i][j].end(), EdgeTripletGreaterByValue<EdgeType>());
