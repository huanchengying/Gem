...
template<typename VertexType, typename EdgeType>
Reachgem<VertexType, EdgeType>::Reachgem(char *input_prefix) {
  string prefix(input_prefix);

  {
    string edge_file_name(prefix + ".reachbility_edge");
  ...
  analysisInfoFile(prefix + ".reachbility_info");
...
template<typename VertexType, typename EdgeType>
Reachgem<VertexType, EdgeType>::~Reachgem() {
...
template<typename VertexType, typename EdgeType>
void Reachgem<VertexType, EdgeType>::analysisInfoFile(string file_name) {
  ...
  while (cell_cnt--) {
    AdjCell<EdgeType> c;
    f >> c.start_vertex >> c.end_vertex >> c.start_dst_vertex >> c.end_dst_vertex >> c.edge_cnt;
    uint64_t cursor1, cursor2;
    f >> cursor1 >> cursor2;
