...
int main(int argc, char ** argv) {
  ...
  for(uint64_t i=1;i<=gem.TotVertexCnt();i++){
      ss.insert(make_pair(i,gem.Vertex(i)));
      if(i>100000){
        set<pair<int,float>>::iterator it=ss.end();
        it--;
        ss.erase(it);
      }
  }
  cerr<<"KNN time: "<<chrono::duration_cast<chrono::milliseconds>(chrono::steady_clock::now() - cur_t).count()/1000.0<<"\n";
  print_reads();
