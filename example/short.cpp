...
struct Worker {
  ...
  void RelaxEdgeTask() const {
    ...
    // printf("local_relaxed: %d %d\n", local_relaxed, controller.has_relaxed);
  }
  ...
  void RAbstractTask() const {
    ...
    if (local_relaxed) controller.has_relaxed = true;
  }
...
int main(int argc, char ** argv) {
  ...
  vector<double> init_t, bridge_t, priority_t, rgraph_t, normal_t, sorted_t, reach_t;
...

    {
      ...
      controller.has_relaxed = true;
      {
        ...
        while (!q.empty()) {
          ...
          VertexType& src_dist = gem.Vertex(x);
      ...
      while (controller.has_relaxed) {
          inum++;
          anum[++ccnt]+=gem.in_activity->get_num();
      ...
      priority_t.push_back(chrono::duration_cast<chrono::milliseconds>(chrono::steady_clock::now() - start_t).count()/1000.0);
      cerr << gem.Vertex(qdst) << "\t"  << priority_t.back() << "\t";
    }
  ...
  if (init_t.size()) {
    ...
    cerr << "Init:\n"
         << "\tmin\t" << init_t[0] << "\n"
         << "\tavg\t" << accumulate(init_t.begin(), init_t.end(), 0.0) / (double)init_t.size() << "\n"
         << "\tmed\t" << init_t[init_t.size()/2] << "\n"
         << "\tmax\t" << init_t.back() << "\n";
  }
  
  if (bridge_t.size()) {
    sort(bridge_t.begin(), bridge_t.end());
    cerr << "Brdige:\n"
         << "\tmin\t" << bridge_t[0] << "\n"
         << "\tavg\t" << accumulate(bridge_t.begin(), bridge_t.end(), 0.0) / (double)bridge_t.size() << "\n"
         << "\tmed\t" << bridge_t[bridge_t.size()/2] << "\n"
         << "\tmax\t" << bridge_t.back() << "\n";
  }
  
  if (priority_t.size()) {
    sort(priority_t.begin(), priority_t.end());
    cerr << "Priority:\n"
         << "\tmin\t" << priority_t[0] << "\n"
         << "\tavg\t" << accumulate(priority_t.begin(), priority_t.end(), 0.0) / (double)priority_t.size() << "\n"
         << "\tmed\t" << priority_t[priority_t.size()/2] << "\n"
         << "\tmax\t" << priority_t.back() << "\n";
  }
  ...
  cerr<< "Iteration: "<<inum/TEST_CASE.size()<<"\n";
  for(int i=0;i<20;i++) cerr<<anum[i]/TEST_CASE.size()<<" ";
